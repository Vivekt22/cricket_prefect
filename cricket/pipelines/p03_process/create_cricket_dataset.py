...
def create_cricket_dataset(
    ...
    df_deliveries = (
        ...
        .with_columns(
            missing_batter_name = when(col('batter_name').is_null()).then(True).otherwise(False),
            missing_bowler_name = when(col('bowler_name').is_null()).then(True).otherwise(False),
            missing_non_striker_name = when(col('non_striker_name').is_null()).then(True).otherwise(False),
            missing_player_out_name = when(col('player_out_name').is_null()).then(True).otherwise(False)
        )
        .with_columns(
    ...
    df_match_info = (
        ...
        .join(
            df_person_names.select(col('person_unique_name').alias('player_of_match'), 
    ...
    df_batting_order = (
        df_cricket_dataset
        .select("innings_id", "delivery", pl.concat_list("batter", "non_striker").alias("batters"), "wickets")
        ...
        .with_columns(batting_order = col("index").rank(method="ordinal").over("innings_id"))
        .drop("index")
    )
    ...
    df_cricket_dataset = (
        df_cricket_dataset
        .join(df_batting_order, how="left", on=["innings_id", "batter"], coalesce=True)
        .rename({"batting_order": "batter_number"})
        .join(df_batting_order, how="left", left_on=["innings_id", "non_striker"], right_on=["innings_id", "batter"], coalesce=True)
