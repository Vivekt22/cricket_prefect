                    ...
                    for delivery_details in inning_details.get('deliveries', []):
                        for delivery, delivery_info in delivery_details.items():
                            delivery_row = {
                                ...
                                'declared': 1 if delivery_info.get('declared', '').lower() == 'yes' else 0,
                                ...
                                'wicket_type': delivery_info.get('wicket', {}).get('kind', ''),
                                'player_out': delivery_info.get('wicket', {}).get('player_out', '')
                            }

                            batch_deliveries.append(delivery_row)

            if not batch_deliveries:
                logger.warning(f"Extract of deliveries for {match_id} skipped due to empty innings")
        except Exception as e:
            logger.warning(f"Extract of deliveries for {match_id} failed due to {e}")

    if batch_deliveries:
        deliveries_df = pl.LazyFrame(batch_deliveries, schema=Conf.schemas.deliveries, orient="row").lazy()
        return deliveries_df
    return None

@flow(task_runner=DaskTaskRunner(cluster_kwargs={"processes": True, "n_workers": 16, "threads_per_worker": 1, "memory_limit": "3GB"}))
def pool_extract_deliveries_details(dump_yaml_files: list[Path], batch_size: int = 100) -> list[pl.DataFrame]:
    logger = get_run_logger()
    logger.info("Starting the pooling process for delivery details")

    batches = [dump_yaml_files[i:i + batch_size] for i in range(0, len(dump_yaml_files), batch_size)]
    results = extract_deliveries_details.map(batches)  # Using map for parallel processing
...
def update_deliveries(df_new_dump_files: pl.LazyFrame|None = None) -> pl.LazyFrame:
    ...
    if df_new_dump_files is None:
        ...
        if df_new_dump_files.select(pl.col("match_id").is_in(df_preprocessed_delivery.select("match_id").collect()).any()).collect().item():
    ...
    dump_yaml_files = [
        ...
        for file 
        in df_new_dump_files.select("match_id").collect().to_series().to_list()
    ]
