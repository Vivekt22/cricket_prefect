...
def extract_registry_details(files_batch: list[Path]):
    ...
    for file in files_batch:
        try:
            ...
            registry_dict = match_data.get('info', {}).get('registry', {}).get('people', {})
    ...
    if batch_registry_info:
        registry_df = pl.DataFrame(batch_registry_info, schema=Conf.schemas.registry, orient="row")
        return registry_df

    return None

@flow(task_runner=DaskTaskRunner(cluster_kwargs={"processes": True, "n_workers": 16, "threads_per_worker": 1, "memory_limit": "3GB"}))
def pool_extract_registry_details(dump_yaml_files: list[Path], batch_size: int = 100) -> list[pl.DataFrame]:
    logger = get_run_logger()
...
def update_registry(df_new_dump_files: pl.LazyFrame|None = None) -> pl.LazyFrame:
    ...
    if df_new_dump_files is None:
        ...
        if df_new_dump_files.select(pl.col("match_id").is_in(df_preprocessed_registry.select("match_id").collect()).any()).collect().item():
