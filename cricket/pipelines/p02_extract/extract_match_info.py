...
def extract_match_info_details(files_batch: List[Path]):
    ...
    if batch_match_info:
        match_info_df = pl.LazyFrame(batch_match_info, schema=Conf.schemas.match_info, orient="row")
...
def pool_extract_match_info_details(dump_yaml_files: List[Path], batch_size: int = 100) -> List[pl.DataFrame]:
    logger = get_run_logger()
    ...
    results = extract_match_info_details.map(batches)  # Using map for parallel processing
...
def update_match_info(df_new_dump_files: pl.LazyFrame|None = None) -> pl.LazyFrame:
    ...
    if df_new_dump_files is None:
        ...
        if df_new_dump_files.select(pl.col("match_id").is_in(df_preprocessed_match_info.select("match_id").collect()).any()).collect().item():
